...
# the config json file, same folder as the compile location
OPTIONS_PATH = os.path.join(BASE_DIR, "configurator_config.json")

# structure.json fodlder same as above
...
def ask_string(prompt, default=""):
    ...
    try:
        ...
        pass
    dialog.title("OKAC Sectorfile Configurator")
...
def ask_yesno(prompt, title="OKAC Sectorfile Configurator"):
    result = None
    dialog = tk.Toplevel()
    try:
        icon_path = resource_path("logo.ico")
...
def collect_basic_config():
    root = tk.Tk()
    root.title("OKAC Sectorfile Configurator")
    try:
        icon_path = resource_path("logo.ico")
        if os.path.exists(icon_path):
    ...
    "profiles_replacements", for example:
        {
          "profiles_replacements": {
            "OKAC_PLACEHOLDER": "OKAC_REAL_VALUE",
